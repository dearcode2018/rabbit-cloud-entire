...
public class SysMenuController extends AbstractController {
	...
	public List<SysMenuEntity> list(){
		...
		// 优化: 转成map，遍历设置上级名称
		final Map<Long, SysMenuEntity> map = toMap(menuList);
		menuList.forEach(x -> {
			SysMenuEntity entity = map.get(x.getParentId());
			if (null != entity) {
				x.setParentName(entity.getName());
			}
		});
		map.clear();
	 ...
	 * @author qianye.zheng
	 */
	private static final Map<Long, SysMenuEntity> toMap(final List<SysMenuEntity> list) {
		final Map<Long, SysMenuEntity> map = new HashMap<>();
		list.forEach(x -> map.put(x.getMenuId(), x));
